...
public class Unit : MonoBehaviour {
	...
	void Update () 
	{
		...
		// set goals
		goalPos = new Vector2(mousePositionInWorld.x, mousePositionInWorld.y);

		//Raycast, avoid obstacles
		if(Physics2D.Raycast(transform.position, velocity, 4))
        {
			Debug.DrawRay(transform.position, this.velocity, Color.white);
			currentForce = new Vector2(Random.Range(0.01f, 0.1f), Random.Range(0.01f, 0.1f));
